...
function parseTime(settings, time) {
    parsedString = "";
    hours = time / 3600;
    minutes = (hours - Math.floor(hours)) * 60;
    seconds = (minutes - Math.floor(minutes)) * 60;
    if (settings.precision == "seconds") {
        parsedString += Math.floor(hours) + "h" + Math.floor(minutes) + "m" + Math.floor(seconds) + "s\n";
    }
    else if (settings.precision == "minutes") {
        parsedString += Math.floor(hours) + "h" + Math.floor(minutes) + "m\n";
    }
    else if (settings.precision == "hours") {
        parsedString += Math.floor(hours) + "h\n";
    }
    else if (settings.precision == "hoursplus") {
        parsedString += hours.toFixed(3) + "h\n";
    }
    ...
    fetch(settings.octoUrl + "/api/job", {
        ...
        .then((out) => {
            ...
            if (out.state.search("Operational") >= 0 && out.progress.completion) {
                if (settings.completionPercent == "yes") {
                    result = Math.floor(out.progress.completion) + "%\n";
                }
                if (settings.showTimeLeft == "yes") {
                    result += parseTime(settings, out.progress.printTimeLeft);
                }
